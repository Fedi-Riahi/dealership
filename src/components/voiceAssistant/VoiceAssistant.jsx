...
const VoiceAssistant = () => {
    ...
    useEffect(() => {
        ...
        if (listening) {
            ...
            recognition.onresult = (event) => {
                ...
                // Check if the phrase contains the word "listing" or any car model/listing title
                const selectedListing = carModels.find(listing => command.includes(listing.listingTitle.toLowerCase().trim()) || command.includes(listing.model.toLowerCase().trim()));
    ...
    return (
        <button
            ...
            className={`fixed bottom-8 right-8 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-full flex items-center ${listening ? 'shadow-md' : ''}`}
        >
            {loading ? (
