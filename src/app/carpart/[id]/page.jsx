...
const PartDetails = () => {
  ...
  return (
    <div className="container mx-auto my-40">
      <div className="flex sm:flex-col md:flex-row items-start gap-10 my-8">
        <div className="">
          <Image src={part.images[0]} alt={part.name} width={700} height={600} className="object-cover bg-gray-100 border-2" />
        ...
        <div className="md:w-1/4 md:ml-8">
          ...
          <h2 className="font-bold text-xl mb-4">{part.name}</h2>
            <h2 className="text-zinc font-semibold text-2xl ">${part.price}</h2>
          ...
          <div className='flex items-center gap-2 mt-2'>
            ...
            {part.compatibleCarModels.map((model, index) => (
              <h2 key={index} className="font-medium text-blue-500">{model}</h2>
            ))}
          </div>
        ...
        <div className=' md:w-[600px] mt-10  px-6 py-8'>
              <h2 className='font-semibold text-xl pb-3 text-blue-500 underline underline-offset-8'>Description</h2>
            ...
            <button className="text-white hover:underline mt-5 w-full text-center mx-auto px-8 py-4 bg-blue-500 flex items-center justify-center gap-2" onClick={() => handleAddToCart(part)}>
              <CiShoppingCart className='h-6 w-6'/>Ajouter au panier
            </button>
          </div>
